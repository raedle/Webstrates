...
module.exports.deleteAssetFromDatabase = (fileName) => {
	if (!fileName || fs.existsSync(path.join(config.uploadsFolder, fileName))) return;
...
const deleteAssetFromFileSystem = (fileName) => {
	...
	return unlink(path.join(config.uploadsFolder, fileName));
};
		...
		db.assets.distinct('fileName', {
			...
			assetsToBeDeleted.forEach(function(asset) {
				...
				promises.push(new Promise(function(resolve, reject) {
					const filePath = path.join(config.uploadsFolder, asset);
			...
			db.assets.insert({
				...
				if (searchable && asset.mimetype === 'text/csv') {
					...
					await searchableAssets.makeSearchable(assetId,
						path.join(config.uploadsFolder, asset.filename));
				}
